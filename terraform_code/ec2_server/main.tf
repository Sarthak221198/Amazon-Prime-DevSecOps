...
resource "aws_route" "public_internet" {
  ...
  destination_cidr_block = "0.0.0.0/0"  # Allows internet access
...
resource "aws_instance" "my-ec2" {
  ...
  subnet_id = aws_subnet.Jenkins-subnet.id
  depends_on = [ aws_subnet.Jenkins-subnet ]
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      private_key = file("./netflix-key.pem") # replace with your key-name 
      user        = "ubuntu"
      host        = self.public_ip
    }
